...
export default function EppShareModal({ isOpen, onClose }: EppShareModalProps) {
    ...
    const [qrPosterShare, setQrPosterShare] = useState<any | null>(null);
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/70 backdrop-blur-sm animate-in fade-in overflow-y-auto">
                    ...
                    {activeShare && (
                        <div className="p-5 bg-gradient-to-br from-indigo-50/70 via-white to-emerald-50/50 dark:from-slate-800/80 dark:via-slate-900 dark:to-emerald-950/20 rounded-2xl border-2 border-indigo-400/40 space-y-4">
                            ...
                            <div className="p-4 bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
                                <div className="flex flex-col sm:flex-row items-center gap-4">
                                    ...
                                    <div className="space-y-1 text-center sm:text-left">
                                        ...
                                        <p className="text-[11px] text-slate-500">
                                            Genera un cartel en hoja vertical A4 listo para colocar en obra, comedor o enviar a inspección.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <button
                                    type="button"
                                    onClick={() => setQrPosterShare(activeShare)}
                                    className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs flex items-center gap-2 whitespace-nowrap active:scale-95 shrink-0"
                                >
            ...
            {qrPosterShare && (
                <EppQrPosterModal
                    isOpen={Boolean(qrPosterShare)}
                    onClose={() => setQrPosterShare(null)}
                    shareUrl={getShareUrl(qrPosterShare.token)}
                    share={qrPosterShare}
                />
            )}
        </div>
