...
export default function ProjectCalendarView({
    ...
    return (
        <div className="bg-card text-card-foreground rounded-[2.5rem] border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col min-h-[600px]">
            {/* Header Toolbar */}
            <div className="p-4 md:p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-4 shrink-0 bg-background/50 text-foreground">
                <div className="flex items-center gap-4">
                    <div className="flex border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden shadow-sm">
                        <button onClick={prevPeriod} className="p-2.5 bg-card text-card-foreground hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                            <ChevronLeft className="w-5 h-5 text-slate-500 dark:text-slate-400" />
                        </button>
                        <button onClick={goToday} className="px-4 py-2 bg-card text-card-foreground border-x border-slate-200 dark:border-slate-700 text-xs font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                            Hoy
                        </button>
                        <button onClick={nextPeriod} className="p-2.5 bg-card text-card-foreground hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
            ...
            {/* Calendar Grid */}
            <div className="flex-1 overflow-auto custom-scrollbar bg-background text-foreground/20">
                <div className="min-w-[800px] h-full flex flex-col">
                    {/* Weekdays Row */}
                    <div className="grid grid-cols-7 border-b border-slate-200 dark:border-slate-700 bg-card text-card-foreground sticky top-0 z-10 shrink-0">
                        {WEEKDAYS.map(day => (
                    ...
                    <div className="grid grid-cols-7 flex-1 auto-rows-fr">
                        {days.map((day, idx) => {
                            ...
                            return (
                                <div 
                                    key={day.toISOString()} 
                                    className={`min-h-[120px] ${viewMode === 'week' ? 'min-h-[400px]' : ''} border-r border-b border-slate-100 dark:border-slate-800/50 p-1 md:p-2 transition-colors hover:bg-slate-50/50 dark:hover:bg-slate-800/50 ${!isCurrentMonth ? 'bg-slate-50/50 dark:bg-slate-900/30' : 'bg-card text-card-foreground'} ${idx % 7 === 0 ? 'border-l' : ''}`}
                                >
                                    ...
                                    <div className="space-y-1 flex flex-col px-0.5">
                                        ...
                                        {dayProjects.length > (viewMode === 'week' ? 20 : 4) && (
                                            <div className="text-[10px] font-black text-slate-400 dark:text-slate-500 text-center py-0.5 uppercase tracking-widest bg-background text-foreground rounded border border-slate-100 dark:border-slate-800">
