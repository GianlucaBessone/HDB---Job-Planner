...
function LogEntry({ log, isExpanded, onToggle, styleGetter }: any) {
    return (
        <div className={`bg-white dark:bg-slate-800 rounded-3xl border transition-all duration-300 overflow-hidden ${isExpanded ? 'border-primary shadow-xl shadow-primary/10' : 'border-slate-100 dark:border-slate-800 shadow-sm hover:border-slate-200 dark:hover:border-slate-700'}`}>
            <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer" onClick={onToggle}>
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-slate-900/50 flex flex-col items-center justify-center border border-slate-100 dark:border-slate-800">
                        <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase leading-none">{format(new Date(log.timestamp), 'MMM')}</p>
                        <p className="text-lg font-black text-slate-700 dark:text-slate-200 leading-tight">{format(new Date(log.timestamp), 'dd')}</p>
                    </div>
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{log.entity}</span>
                        </div>
                        <p className="text-sm font-bold text-slate-700 dark:text-slate-200 underline underline-offset-4 decoration-primary/20">
                ...
                <div className="flex items-center gap-6">
                    <div className="text-right hidden md:block">
                        ...
                        <p className="text-xs font-bold text-slate-500 dark:text-slate-400 font-mono">{log.entityId?.slice(0, 12)}...</p>
                    </div>
                    <div className="text-right">
                        <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1">Hora</p>
                        <p className="text-sm font-bold text-slate-700 dark:text-slate-200">{format(new Date(log.timestamp), 'HH:mm:ss')}</p>
                    </div>
                    <div className="p-2 rounded-full hover:bg-slate-50 dark:hover:bg-slate-800/80 transition-colors">
            ...
            {isExpanded && (
                <div className="p-8 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-800 grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in slide-in-from-top-4 duration-300">
                    <div className="space-y-4">
                        <h5 className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1">Estado Anterior</h5>
                        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 overflow-x-auto">
                            <pre className="text-[10px] font-medium text-slate-500 dark:text-slate-400 whitespace-pre-wrap">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-primary/20 shadow-sm overflow-x-auto relative">
                             <div className="absolute top-2 right-2 bg-primary/10 text-primary p-1.5 rounded-lg">
                                <ArrowRight className="w-3 h-3" />
                             </div>
                            <pre className="text-[10px] font-bold text-slate-800 dark:text-slate-100 whitespace-pre-wrap">
