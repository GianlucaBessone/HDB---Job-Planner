...
export default function DocumentDetailModal({
  ...
  const [tab, setTab] = useState<
    "info" | "lms" | "versiones" | "reglas" | "checklist"
  >("info");

  // Checklist template state
  const [checklistItems, setChecklistItems] = useState<
    { descripcion: string; esObligatorio: boolean }[]
  >([]);
  ...
  const [agreement, setAgreement] = useState<"ACUERDO" | "DESACUERDO" | null>(
    null,
  );
  ...
  const [pendingSaveType, setPendingSaveType] = useState<
    "digital" | "requirements" | "quiz" | "checklist" | null
  >(null);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        setCustomAbbrevs(JSON.parse(saved));
      } catch (e) {}
    }
  }, []);
  ...
  const handleSaveRequirements = () => {
    ...
    setPendingSaveData({
      ...
      validezMeses: editReqCapacitacion
        ? editValidezMeses
  ...
  const executeVersionedSave = async () => {
    ...
    try {
      ...
      if (revOp)
        updateBody.revisadorNombre = revOp.nombreCompleto || revOp.nombre;
      ...
      if (versionAction === "none") {
        ...
        if (pendingSaveType === "digital")
          desc = "Actualización de procedimiento digital";
        if (pendingSaveType === "requirements")
          desc = "Actualización de requerimientos LMS";
        if (pendingSaveType === "quiz")
          desc = "Actualización de evaluación LMS";
        if (pendingSaveType === "checklist")
          desc = "Actualización de checklist";
      ...
      if (!res.ok) {
        ...
        showToast(
          err.error || "Error al guardar cambios en documento",
          "error",
        );
      ...
      if (versionAction !== "none") {
        const payload: any = {
          ...
          motivoCambio:
            ...
            (pendingSaveType === "checklist"
              ? "Actualización de plantilla de checklist"
        ...
        const vRes = await safeApiRequest(
          `/api/documentos/${documentId}/versions`,
          {
            method: "POST",
            body: JSON.stringify(payload),
          },
        ...
        if (vRes.ok) {
          showToast(
            `Versión ${versionAction === "mayor" ? "mayor" : "menor"} creada con éxito. Documento enviado a revisión.`,
            "success",
          );
        } else {
        ...
        if (pendingSaveType === "checklist") {
          if (!doc.versions || doc.versions.length === 0) {
            ...
            await safeApiRequest(
              `/api/documentos/${documentId}/versions/${latestVersion.id}`,
              {
                method: "PUT",
                body: JSON.stringify({
            ...
            showToast(
              "Plantilla de checklist actualizada sin incrementar versión",
              "success",
            );
          }
        } else {
  ...
  const handleWorkflowSubmit = async () => {
    ...
    if (agreement === "ACUERDO") {
      ...
      if (!comments.trim()) {
        alert(
          "Debe proporcionar una recomendación de cambios o modificaciones cuando está en desacuerdo",
        );
        return;
      }
    }
    ...
    try {
      const isRevisadorPending =
        user &&
        doc.revisadorId === user.id &&
        doc.workflowState?.revisadorStatus === "pending";
      const isAprobadorPending =
        user &&
        doc.aprobadorId === user.id &&
        doc.workflowState?.aprobadorStatus === "pending";
      ...
      if (res.ok) {
        showToast(
          status === "approved"
            ...
            : "Observaciones enviadas con éxito",
          "success",
        );
        // Reset local states
        setAgreement(null);
  ...
  const loadDocument = async (forceRefresh = false) => {
    ...
    if (cachedDoc) {
      ...
      if (
        cachedDoc.descripcion &&
        cachedDoc.descripcion.trim().startsWith("{")
      ) {
        try {
      ...
      if (
        cachedDoc.workflowState &&
        typeof cachedDoc.workflowState === "object" &&
        Array.isArray(cachedDoc.workflowState.cuestionario)
      ) {
    ...
    try {
      ...
      const optPromise = cachedActivityOptions
        ? Promise.resolve(null)
        : safeApiRequest("/api/config/options");
      const tagsPromise = cachedProjectTags
        ? Promise.resolve(null)
        : safeApiRequest("/api/config/tags");
      const allDocsPromise = cachedAllDocs
        ? Promise.resolve(null)
        : safeApiRequest("/api/documentos");
      const opsPromise = cachedOperators
        ? Promise.resolve(null)
      ...
      if (docRes.ok) {
        ...
        if (
          data.workflowState &&
          typeof data.workflowState === "object" &&
          Array.isArray(data.workflowState.cuestionario)
        ) {
  ...
  const deleteRule = async (ruleId: string) => {
    try {
      await safeApiRequest(
        `/api/documentos/${documentId}/reglas?ruleId=${ruleId}`,
        {
          method: "DELETE",
        },
      );
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        const res = await safeApiRequest(
          `/api/documentos/${documentId}/upload`,
          {
            method: "POST",
            body: JSON.stringify(payload),
          },
  ...
  const addQuizQuestion = () => {
    ...
    if (newOptions.some((opt) => !opt.trim())) {
      return showToast(
        "Todas las opciones de respuesta son obligatorias",
        "error",
      );
    }
  ...
  const handleGenerateQuizWithAi = async () => {
    ...
    try {
      ...
      if (
        data.success &&
        data.training &&
        Array.isArray(data.training.cuestionario)
      ) {
        ...
        showToast(
          "Cuestionario generado con éxito mediante IA (edítelo y guárdelo)",
          "success",
        );
      } else {
        showToast(
          "La IA no pudo formatear el cuestionario correctamente.",
          "error",
        );
      }
    } catch (err: any) {
          ...
          <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Ha modificado el documento. Seleccione cómo proceder con el
                versionado.
              </p>
            </div>
            <div className="p-6 space-y-5">
              <div className="space-y-3">
                <label className="flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors bg-white hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-700/50 border-slate-200 dark:border-slate-700">
                  <input
                    ...
                    name="versionAction"
                    value="mayor"
                    checked={versionAction === "mayor"}
                    onChange={() => setVersionAction("mayor")}
                    className="mt-1"
                  />
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
                      Incrementar Versión MAYOR (+1.0)
                    </p>
                    <p className="text-xs text-slate-500">
                      Cambios estructurales, nuevo alcance, o reescritura
                      significativa.
                    </p>
                  </div>
                </label>
                <label className="flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors bg-white hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-700/50 border-slate-200 dark:border-slate-700">
                  <input
                    ...
                    name="versionAction"
                    value="menor"
                    checked={versionAction === "menor"}
                    onChange={() => setVersionAction("menor")}
                    className="mt-1"
                  />
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
                      Incrementar Versión MENOR (+0.1)
                    </p>
                    <p className="text-xs text-slate-500">
                      Correcciones ortográficas, actualizaciones menores o
                      adición de detalles sin afectar el procedimiento general.
                    </p>
                  </div>
                </label>
                <label className="flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors bg-white hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-700/50 border-slate-200 dark:border-slate-700">
                  <input
                    ...
                    name="versionAction"
                    value="none"
                    checked={versionAction === "none"}
                    onChange={() => setVersionAction("none")}
                    className="mt-1"
                  />
                  <div>
                    <p className="text-sm font-bold text-slate-800 dark:text-slate-100">
                    ...
                    <p className="text-xs text-slate-500">
                      Solo aplicable para correcciones mínimas (typos) que NO
                      alteran en absoluto el contenido normativo. Requiere
                      justificación.
                    </p>
                  </div>
                </label>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
                  Justificación del cambio / Motivo{" "}
                  {versionAction === "none" && (
              ...
              <div className="space-y-6 max-w-3xl mx-auto">
                ...
                {(() => {
                  ...
                  try {
                    if (
                      doc.descripcion &&
                      doc.descripcion.trim().startsWith("{")
                    ) {
                  ...
                  const canEdit = ["supervisor", "admin", "qa"].includes(
                    (user?.role || "").toLowerCase(),
                  );
                  const youtubeUrl =
                    isDigital && digitalData?.videoUrl
                      ? getYoutubeEmbedUrl(digitalData.videoUrl)
                      : null;
                  const DEFAULT_ABBREVIATIONS = [
                    {
                      term: "EPP",
                      definition: "Equipos de Protección Personal",
                    },
                    ...
                    {
                      term: "ISO",
                      definition: "Organización Internacional de Normalización",
                    },
                    {
                      term: "QA",
                      definition:
                    ...
                    },
                    {
                      term: "HSE",
                      definition:
                        "Health, Safety and Environment (Higiene, Seguridad y Medio Ambiente)",
                    },
                    {
                      term: "SOP",
                      definition:
                    ...
                    {
                      term: "SST",
                      definition: "Seguridad y Salud en el Trabajo",
                    },
                  ];
                    ...
                    return (
                      <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl border-2 border-indigo-300 dark:border-indigo-700 space-y-6 shadow-xl shadow-indigo-500/5">
                        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <h3 className="font-black text-slate-800 dark:text-slate-100">
                              Editor de Procedimiento Digital
                            </h3>
                          </div>
                          <div className="flex gap-2">
                            <button
                              onClick={() => setIsEditingDigital(false)}
                              className="px-3 py-1.5 text-xs font-bold text-slate-500 hover:text-slate-700 bg-slate-100 dark:bg-slate-700 rounded-lg"
                            >
                              Cancelar
                            </button>
                            <button
                        ...
                        <div className="space-y-1.5">
                          ...
                          <textarea
                            value={editedDigitalData.objetivo}
                            onChange={(e) =>
                              setEditedDigitalData({
                                ...editedDigitalData,
                        ...
                        <div className="space-y-1.5">
                          ...
                          <textarea
                            value={editedDigitalData.alcance}
                            onChange={(e) =>
                              setEditedDigitalData({
                                ...editedDigitalData,
                        ...
                        <div className="space-y-1.5">
                          ...
                          <textarea
                            value={editedDigitalData.desarrollo}
                            onChange={(e) =>
                              setEditedDigitalData({
                                ...editedDigitalData,
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            type="url"
                            value={editedDigitalData.videoUrl}
                            onChange={(e) =>
                              setEditedDigitalData({
                                ...editedDigitalData,
                        ...
                        <div className="space-y-3 border-t border-slate-200 dark:border-slate-700 pt-4">
                          ...
                          {editedDigitalData.definiciones &&
                            editedDigitalData.definiciones.length > 0 && (
                              <div className="space-y-1.5">
                                {editedDigitalData.definiciones.map(
                                  (d: any, i: number) => (
                                    <div
                                      key={i}
                          ...
                          <div className="bg-indigo-50 dark:bg-indigo-950/30 p-3 rounded-xl space-y-2">
                            <p className="text-[10px] font-black text-indigo-500 uppercase tracking-wider">
                              Seleccionar abreviatura conocida
                            </p>
                          ...
                          <label className="flex items-center gap-1.5 text-[10px] text-slate-500">
                            <input
                              ...
                              onChange={(e) =>
                                setSaveToFrequents(e.target.checked)
                              }
                        ...
                        <div className="space-y-3 border-t border-slate-200 dark:border-slate-700 pt-4">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-wider">
                            7. Referencias a Documentos Internos
                          </label>
                          {editedDigitalData.referencias &&
                            editedDigitalData.referencias.length > 0 && (
                              <div className="space-y-1.5">
                                {editedDigitalData.referencias.map(
                                  (r: any, i: number) => (
                                    <div
                                      key={i}
                                      className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900/30 px-3 py-2 rounded-lg text-xs"
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              onClick={() => {
                                if (!customRefText) return;
                                setEditedDigitalData({
                                  ...editedDigitalData,
                                  referencias: [
                                    ...(editedDigitalData.referencias || []),
                                    { codigo: "EXT", titulo: customRefText },
                                  ],
                                });
                                setCustomRefText("");
                              }}
                              className="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 flex-shrink-0"
                            >
                              + Ref
                            </button>
                          </div>
                    ...
                    return (
                      <div className="space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-2">
                            {canEdit && (
                              <button
                                onClick={() => {
                                  setEditedDigitalData({ ...digitalData });
                                  setIsEditingDigital(true);
                                }}
                                className="px-3 py-1.5 text-xs font-bold text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors"
                              >
                                ✏️ Editar
                              </button>
                            )}
                        ...
                        {[
                          {
                            title: "2. Objetivo",
                            content: digitalData.objetivo,
                          },
                          { title: "3. Alcance", content: digitalData.alcance },
                          {
                            title: "4. Desarrollo de la Actividad",
                      ...
                      {canEdit && (
                        <button
                          onClick={() => {
                            ...
                            handleSaveDigitalData();
                            setTimeout(() => {
                              setIsEditingDigital(true);
                              loadDocument(true);
                            }, 500);
                          }}
                          className="w-full py-3 border-2 border-dashed border-indigo-300 dark:border-indigo-700 rounded-2xl text-sm font-bold text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors flex items-center justify-center gap-2"
                        >
                          <Sparkles className="w-4 h-4" /> Convertir a
                          Procedimiento Digital (formulario interactivo)
                        </button>
                      )}
                    </div>
                  );
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                    ...
                    <p
                      className={`font-bold text-sm ${
                        doc.nivelCriticidad === "critico" ||
                        doc.nivelCriticidad === "critica"
                          ? "text-red-600"
                          : doc.nivelCriticidad === "alto" ||
                              doc.nivelCriticidad === "alta"
                            ? "text-amber-600"
                            : doc.nivelCriticidad === "medio" ||
                                doc.nivelCriticidad === "media"
                              ? "text-blue-600"
                              : "text-slate-600"
                      }`}
                    >
                  ...
                  <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                    ...
                    <p className="font-bold text-lg text-indigo-600">
                      v{doc.versionMayor}.{doc.versionMenor}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                      ...
                      <p className="font-bold text-sm text-blue-700 dark:text-blue-300">
                        {doc.revisadorNombre || "Sin asignar"}
                      </p>
                    </div>
                    <div className="text-center p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl">
                      ...
                      <p className="font-bold text-sm text-emerald-700 dark:text-emerald-300">
                        {doc.aprobadorNombre || "Sin asignar"}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(doc.tags as string[]).map((tag: string) => (
                        <span
                          key={tag}
                          className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 px-3 py-1 rounded-lg text-xs font-bold border border-indigo-100 dark:border-indigo-800"
                        >
                          {tag}
                        </span>
                      ))}
                ...
                <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-4">
                  <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700 pb-2">
                    ...
                    {(user?.role?.toLowerCase() === "supervisor" ||
                      ...
                      <span className="text-[10px] font-black uppercase text-indigo-500 bg-indigo-50 dark:bg-indigo-950/30 px-2 py-0.5 rounded">
                        Editor
                      </span>
                    )}
                  </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-4">
                        <label className="flex items-center gap-2.5 cursor-pointer group flex-1">
                          <input
                            ...
                            onChange={(e) =>
                              setEditReqLectura(e.target.checked)
                            }
                        ...
                        <label className="flex items-center gap-2.5 cursor-pointer group flex-1">
                          <input
                            ...
                            onChange={(e) =>
                              setEditReqCapacitacion(e.target.checked)
                            }
                            className="w-4 h-4 rounded border-slate-300 dark:border-slate-600 text-indigo-600 focus:ring-indigo-500"
                          />
                          <span className="text-xs font-bold text-slate-700 dark:text-slate-200">
                            Requiere Capacitación Obligatoria (LMS)
                          </span>
                        </label>
                      </div>
                      ...
                      {editReqCapacitacion && (
                        <div className="space-y-1.5 animate-in slide-in-from-top-1 duration-150 pl-4 border-l-2 border-indigo-500">
                          <label className="block text-[10px] font-black text-slate-500 uppercase tracking-wider">
                            Periodo de Validez / Vencimiento (en meses)
                          </label>
                          <input
                            type="number"
                            min="1"
                  ...
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-6">
                    <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-3">
                      ...
                      <h3 className="font-bold text-slate-800 dark:text-slate-100">
                        Estado de Firmas y Flujo de Aprobación
                      </h3>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      >
                        <div>
                          ...
                          <p className="font-bold text-sm text-slate-700 dark:text-slate-200 mt-1">
                            {doc.workflowState?.editorName ||
                              doc.createdByName ||
                              "Editor"}
                          </p>
                          <p className="text-[10px] text-slate-500">
                            {doc.workflowState?.editorPosition ||
                              doc.workflowState?.creatorPosition ||
                              ""}
                          </p>
                          {(doc.workflowState.creatorStatus === "approved" ||
                            doc.workflowState.creatorSignature) && (
                        ...
                        <div className="mt-3">
                          ...
                          <p className="text-[9px] text-slate-400 mt-1">
                            {doc.workflowState.creatorSignatureDate
                      ...
                      {doc.revisadorId && (
                        <div
                          className={`p-4 rounded-xl border flex flex-col justify-between min-h-[140px] ${
                            doc.workflowState.revisadorStatus === "approved"
                              ? "bg-emerald-50/30 dark:bg-emerald-950/10 border-emerald-200 dark:border-emerald-900/50"
                        ...
                        >
                          <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                              Verificador / Revisado Por
                            </p>
                            <p className="font-bold text-sm text-slate-700 dark:text-slate-200 mt-1">
                              {doc.revisadorNombre || "Sin asignar"}
                            </p>
                            <p className="text-[10px] text-slate-500">
                              {doc.workflowState?.revisadorPosition || ""}
                            </p>
                            <span
                              className={`inline-block text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded mt-2 ${
                                doc.workflowState.revisadorStatus === "approved"
                                  ? "bg-emerald-100 text-emerald-700"
                                  : doc.workflowState.revisadorStatus ===
                                      "rejected"
                      ...
                      {doc.aprobadorId && (
                        <div
                          className={`p-4 rounded-xl border flex flex-col justify-between min-h-[140px] ${
                            doc.workflowState.aprobadorStatus === "approved"
                              ? "bg-emerald-50/30 dark:bg-emerald-950/10 border-emerald-200 dark:border-emerald-900/50"
                        ...
                        >
                          <div>
                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                              Autorizante / Aprobado Por
                            </p>
                            <p className="font-bold text-sm text-slate-700 dark:text-slate-200 mt-1">
                              {doc.aprobadorNombre || "Sin asignar"}
                            </p>
                            <p className="text-[10px] text-slate-500">
                              {doc.workflowState?.aprobadorPosition || ""}
                            </p>
                            <span
                              className={`inline-block text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded mt-2 ${
                                doc.workflowState.aprobadorStatus === "approved"
                                  ? "bg-emerald-100 text-emerald-700"
                                  : doc.workflowState.aprobadorStatus ===
                                      "rejected"
                                    ? "bg-red-100 text-red-700"
                                    : "bg-amber-100 text-amber-700"
                              }`}
                    ...
                    {user &&
                      (() => {
                        ...
                        const hasQuiz =
                          !doc.requiereCapacitacion ||
                          (doc.workflowState?.cuestionario &&
                            Array.isArray(doc.workflowState.cuestionario) &&
                            doc.workflowState.cuestionario.length > 0);
                            ...
                            <div className="mt-6 bg-amber-50/50 dark:bg-amber-950/10 p-5 rounded-2xl border border-amber-200 dark:border-amber-900/50 space-y-3">
                              <div className="flex items-center gap-2 text-amber-800 dark:text-amber-200">
                                ...
                                <span className="font-bold text-sm">
                                  Firma Bloqueada: Requisitos Pendientes
                                </span>
                              </div>
                              <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">
                                Este documento no se puede revisar ni aprobar
                                todavía porque no se ha configurado su
                                evaluación LMS:
                              </p>
                              <ul className="list-disc pl-5 text-xs text-slate-600 dark:text-slate-400 space-y-1">
                                {!hasQuiz && (
                                  <li>
                                    Falta configurar las preguntas de la
                                    evaluación (Multiple Choice).
                                  </li>
                                )}
                              </ul>
                              <p className="text-xs text-slate-500 font-bold italic pt-1">
                                Vaya a la pestaña "LMS & Capacitación" para
                                completar el cuestionario y desbloquear las
                                firmas.
                              </p>
                            </div>
                          );
                        ...
                        return (
                          <div className="mt-6 bg-indigo-50/50 dark:bg-indigo-950/10 p-5 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 space-y-4">
                            <div className="flex items-center gap-2 text-indigo-900 dark:text-indigo-100">
                              ...
                              <span className="font-bold text-sm">
                                Panel de Firmante: Requiere tu Acción
                              </span>
                            </div>
                            <p className="text-xs text-slate-500">
                              Has sido asignado como{" "}
                              {isRevisador
                                ? "Verificador (Revisador)"
                                : "Autorizante (Aprobador)"}{" "}
                              para este documento. Por favor revisa el documento
                              y manifiesta tu acuerdo o desacuerdo.
                            </p>
                            ...
                            {agreement === "DESACUERDO" && (
                              <div className="space-y-3 animate-in fade-in duration-200">
                                <label className="block text-xs font-bold text-slate-500 uppercase">
                                  Recomendación de cambios o modificaciones{" "}
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">
                      Reglas de Asignación Automática a OS
                    </h3>
                    <p className="text-sm text-slate-500">
                      Define en qué Órdenes de Servicio este documento aparecerá
                      obligatoriamente.
                    </p>
                  </div>
                  <button
                ...
                {doc.applicabilityRules?.length === 0 ? (
                  <div className="text-center py-12 bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 border-dashed">
                    ...
                    <p className="text-sm text-slate-400">
                      Este documento no se asignará automáticamente a ninguna OS
                      por ahora.
                    </p>
                  </div>
                ) : (
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <p className="font-bold text-slate-700 dark:text-slate-200 text-sm mt-0.5">
                              {rule.tipoActividad
                                ? `Solo para OS de tipo: ${rule.tipoActividad}`
                                : "Para cualquier tipo de OS"}
                            </p>
                          </div>
                          {rule.tagsRequeridos &&
                            Array.isArray(rule.tagsRequeridos) &&
                            rule.tagsRequeridos.length > 0 && (
                              <div>
                                <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                                  Etiquetas de Proyecto Requeridas
                                </p>
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {rule.tagsRequeridos.map((t: string) => (
                                    <span
                                      key={t}
                                      className="bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-[10px] font-bold px-2 py-0.5 rounded border border-indigo-100/50 dark:border-indigo-800/50"
                                    >
                                      {t}
                                    </span>
                                  ))}
                          ...
                          <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100 dark:border-slate-700">
                            ...
                            {rule.generaChecklist && (
                              <span className="bg-emerald-100 text-emerald-700 text-xs font-bold px-2 py-1 rounded flex items-center gap-1">
                                <CheckCircle2 className="w-3 h-3" /> Genera
                                Checklist
                              </span>
                            )}
                          </div>
                ...
                <div className="flex justify-between items-center bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-2xl border border-indigo-100 dark:border-indigo-800">
                  <div>
                    ...
                    <p className="text-xs text-indigo-700 dark:text-indigo-300 mt-1">
                      Estos pasos se exigirán a los técnicos si la Regla de
                      Asignación tiene "Genera Checklist" activado.
                    </p>
                  </div>
                  <button
                    ...
                    {checklistItems.map((item, idx) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() =>
                            setChecklistItems(
                              checklistItems.filter((_, i) => i !== idx),
                            )
                          }
                  ...
                  <div className="p-3 bg-slate-50 dark:bg-slate-900/50">
                    <button
                      onClick={() =>
                        setChecklistItems([
                          ...checklistItems,
                          { descripcion: "", esObligatorio: true },
                        ])
                      }
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-4 shadow-sm animate-in fade-in duration-200">
                  <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-bold text-base text-slate-800 dark:text-slate-100">
                        1. Material Didáctico y Control Documental
                      </h3>
                    </div>
                    <div>
                      {doc.versions?.[0]?.files?.some(
                        ...
                        <span className="px-3 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-950/30 dark:text-emerald-400 flex items-center gap-1.5">
                          <CheckCircle2 className="w-3.5 h-3.5" /> Archivo
                          Subido
                        </span>
                      ) : (
                  ...
                  <p className="text-xs text-slate-500 leading-relaxed">
                    Opcionalmente, puede subir un documento de respaldo en
                    formato Word o PDF para acompañar el contenido digital de
                    esta versión del procedimiento.
                  </p>
                  ...
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-5 shadow-sm">
                    <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-700 pb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-base text-slate-800 dark:text-slate-100">
                          2. Cuestionario de Evaluación (LMS Multiple Choice)
                        </h3>
                      </div>
                      <div>
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed">
                      Dado que este documento requiere capacitación obligatoria,
                      debe estructurar una evaluación de opción múltiple. Cada
                      técnico asignado deberá aprobar esta evaluación antes de
                      considerarse apto para la operación técnica.
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center bg-indigo-50/40 dark:bg-indigo-950/10 p-4 rounded-2xl border border-indigo-100/50 dark:border-indigo-900/20 gap-3">
                      <div className="space-y-0.5">
                        <p className="text-xs font-bold text-indigo-950 dark:text-indigo-100 flex items-center gap-1">
                          ...
                          Cuestionario Inteligente con IA
                        </p>
                        <p className="text-[10px] text-slate-500 dark:text-slate-400">
                          Genere automáticamente un borrador del cuestionario a
                          partir de la Información General, video insertado y
                      ...
                      <div className="flex justify-between items-center bg-indigo-50/50 dark:bg-indigo-950/10 p-4 rounded-xl border border-indigo-100/50 dark:border-indigo-900/30">
                        <div>
                          <p className="text-xs font-bold text-slate-700 dark:text-slate-300">
                            ¿Desea agregar más preguntas?
                          </p>
                          <p className="text-[10px] text-slate-400">
                            Puede agregar múltiples preguntas de opción múltiple
                            a la evaluación.
                          </p>
                        </div>
                        <button
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 pt-2">
                          <div className="flex-1 max-w-xs">
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setNewCorrectIndex(parseInt(e.target.value))
                              }
                              className="w-full bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-xs outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800 dark:text-slate-100"
                            >
                              {newOptions.map((opt, oIdx) => (
                    ...
                    {quizQuestions.length > 0 ? (
                      <div className="space-y-3">
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-300 block">
                          Preguntas del Cuestionario ({quizQuestions.length})
                        </span>
                        <div className="divide-y divide-slate-100 dark:divide-slate-800 border border-slate-150 dark:border-slate-800 rounded-xl overflow-hidden bg-slate-50/50 dark:bg-slate-900/10">
                          {quizQuestions.map((q, idx) => (
                            <div
                              key={idx}
                              className="p-4 flex justify-between gap-4 items-start hover:bg-slate-50 dark:hover:bg-slate-900/30 transition-colors"
                            >
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pl-7">
                                  {q.options?.map((opt, oIdx) => (
                                    ...
                                    >
                                      ...
                                      {opt}
                                      {oIdx === q.correctAnswerIndex && (
                                        <span className="ml-1 text-[9px] font-bold text-emerald-600 uppercase tracking-widest">
                                          (Correcta)
                                        </span>
                                      )}
                                    </div>
                      ...
                      <div className="text-center py-6 border border-dashed border-slate-250 dark:border-slate-800 rounded-2xl bg-slate-50/30 dark:bg-slate-900/5">
                        ...
                        <p className="text-xs font-medium text-slate-400">
                          No hay preguntas agregadas todavía. Complete el
                          formulario anterior.
                        </p>
                      </div>
                    )}
                    ...
                    <div className="flex justify-end pt-2 border-t border-slate-100 dark:border-slate-700">
                      ...
                      >
                        <Save className="w-4 h-4" /> Guardar Cuestionario en
                        Servidor
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-5 shadow-sm">
                    <div className="border-b border-slate-100 dark:border-slate-700 pb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-base text-slate-800 dark:text-slate-100">
                          3. Estado de Avance de Técnicos Asignados (LMS
                          Tracker)
                        </h3>
                      </div>
                    </div>
                    ...
                    {(() => {
                      const activeTrainings =
                        doc.trainings?.filter(
                          (t: any) =>
                            t.operator?.activo && t.estado !== "obsoleto",
                        ) || [];
                      const total = activeTrainings.length;
                      const approved = activeTrainings.filter(
                        (t: any) => t.estado === "aprobado",
                      ).length;
                      const failed = activeTrainings.filter(
                        (t: any) => t.estado === "reprobado",
                      ).length;
                        ...
                        <div className="space-y-5">
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            ...
                            <div className="bg-slate-50 dark:bg-slate-900/30 p-3.5 rounded-xl border border-slate-100 dark:border-slate-800 text-center animate-in zoom-in-95 duration-200">
                              ...
                              <span className="text-lg font-black text-slate-600 dark:text-slate-300 block">
                                {pending}
                              </span>
                              <span className="text-[10px] text-slate-405 font-bold">
                                {pendingPct}%
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <span
                            className={`ml-3 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest ${
                              isVigente
                                ? "bg-emerald-100 text-emerald-700"
                                : "bg-slate-100 text-slate-500"
                            }`}
                        ...
                        <div className="flex flex-col items-end gap-1">
                          ...
                          {!isVigente && (
                            <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                              <GitCompare className="w-3 h-3" /> Click para
                              comparar con la versión vigente
                            </span>
                          )}
                        </div>
                      </div>
                ...
                {projectTags.length > 0 && (
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-1">
                      La regla se aplicará si el proyecto tiene alguna de estas
                      etiquetas.
                    </p>
                  </div>
                )}
