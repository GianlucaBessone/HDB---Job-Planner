...
export default function LmsTab({ user }: { user: any }) {
    ...
    const [supervisorTab, setSupervisorTab] = useState<'internal' | 'external'>('internal');
    const [externalCerts, setExternalCerts] = useState<any[]>([]);
    const [loadingCerts, setLoadingCerts] = useState(false);
    const [selectedCert, setSelectedCert] = useState<any>(null);
    const [certFilter, setCertFilter] = useState<'all' | 'pendiente' | 'aprobado' | 'rechazado'>('all');
    ...
    const openValidationModal = (cert: any) => {
        ...
        setCertFechaEmision(cert.fechaEmision ? cert.fechaEmision.split('T')[0] : '');
    };

    const handleValidateCertificate = async (estado: 'aprobado' | 'rechazado') => {
        ...
        try {
            ...
            if (res.ok) {
                showToast(`Certificado ${estado === 'aprobado' ? 'aprobado' : 'rechazado'} con éxito`, 'success');
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h3 className="text-lg font-black text-slate-800 dark:text-slate-100">
                            {!isTech ? 'Formación y Certificaciones' : 'Centro de Formación Integral'}
                        </h3>
                        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">
                            {isTech ? 'Completa tus capacitaciones y evalúa tus competencias' : 'Gestión y validación de planes de formación y certificados de operadores'}
                        </p>
                    </div>
                </div>
                {!isTech && (
            ...
            {!isTech && (
                <div className="flex border-b border-slate-200 dark:border-slate-800">
                    ...
                    >
                        ...
                        Validar Certificados Externos ({externalCerts.filter(c => c.estado === 'pendiente').length} pendientes)
                    </button>
                </div>
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {trainings.length === 0 && (
                        <div className="col-span-full bg-slate-50 dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-500">
                            ...
                            <p className="text-xs font-medium text-slate-500 mt-1">No hay cursos ni capacitaciones obligatorias asignadas actualmente.</p>
                        </div>
                    )}
                </div>
            )}
                ...
                <div className="space-y-6">
                    ...
                    {loadingCerts ? (
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-500">
                            ...
                            <h4 className="text-sm font-black text-slate-700 dark:text-slate-300">Sin Certificados</h4>
                            <p className="text-xs font-medium text-slate-500 mt-1">No se encontraron certificados externos en esta categoría.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredCerts.map((cert) => (
                                <div key={cert.id} className="bg-white dark:bg-slate-800 rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 flex flex-col justify-between space-y-6 hover:shadow-md transition-shadow">
                                    <div className="space-y-3">
                                        <div className="flex justify-between items-start">
                                            <span className={`text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full ${
                                        ...
                                        <div className="space-y-1">
                                            <h4 className="text-sm font-black text-slate-800 dark:text-slate-100 line-clamp-2">{cert.nombreCurso}</h4>
                                            <p className="text-xs font-bold text-slate-500">Presentado por: {cert.operator?.nombreCompleto}</p>
                                    ...
                                    <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-700/50">
                                        <div className="flex justify-between text-xs font-bold text-slate-500">
                                            <span>Horas:</span>
                                            <span className="text-slate-800 dark:text-slate-100">{cert.horas ? `${cert.horas} hs` : 'N/D'}</span>
                                        </div>
                                        <div className="flex justify-between text-xs font-bold text-slate-500">
                                            <span>Fecha de Emisión:</span>
                                            <span className="text-slate-800 dark:text-slate-100">
                                                {cert.fechaEmision ? new Date(cert.fechaEmision).toLocaleDateString() : 'N/D'}
                                            </span>
                                        </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-4xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] border border-slate-100 dark:border-slate-700">
                        ...
                        <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
                            <div>
                                ...
                                <p className="text-xs text-slate-400 mt-0.5">Operador: {selectedCert.operator?.nombreCompleto}</p>
                            </div>
                            <button 
                                onClick={() => { if (!validatingCert) setSelectedCert(null); }}
                        ...
                        <div className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-2">
                            ...
                            <div className="p-6 border-r border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/10 flex flex-col justify-center min-h-[300px]">
                                {selectedCert.archivoUrl && selectedCert.archivoUrl.startsWith('data:image/') ? (
                                    <img src={selectedCert.archivoUrl} alt="Certificado" className="max-w-full max-h-[50vh] object-contain mx-auto rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm" />
                                ) : selectedCert.archivoUrl && selectedCert.archivoUrl.startsWith('data:application/pdf') ? (
                                    <iframe src={selectedCert.archivoUrl} className="w-full h-[50vh] rounded-xl border border-slate-200 dark:border-slate-700" />
                                ) : (
                                    <div className="p-8 text-center bg-slate-100 dark:bg-slate-900 rounded-2xl text-slate-400 text-xs">
                                        No hay vista previa disponible o el formato no es compatible.
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="p-6 space-y-4 flex flex-col justify-between">
                                <div className="space-y-3">
                                    {selectedCert.aiData && (
                                        <div className="bg-indigo-50/50 dark:bg-indigo-950/20 p-4 rounded-2xl border border-indigo-100/80 dark:border-indigo-900/50">
                                            <p className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-1.5 mb-2">
                                                <Sparkles className="w-3.5 h-3.5" /> Metadatos del Análisis de IA Gemini
                                            </p>
                                            <p className="text-xs text-slate-500 leading-relaxed">
                                                Gemini ha escaneado la imagen/PDF y extrajo la siguiente información del certificado. Puede corregir cualquier campo a continuación.
                                            </p>
                                        </div>
                                    )}

                                    <div className="space-y-1">
                                        <label className="text-xs font-bold text-slate-500">Nombre del Curso</label>
                                        <input
                                            type="text"
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="space-y-1">
                                            <label className="text-xs font-bold text-slate-500">Duración (Horas)</label>
                                            <input
                                                type="number"
                                                value={certHoras}
                                        ...
                                        <div className="space-y-1">
                                            <label className="text-xs font-bold text-slate-500">Fecha de Emisión</label>
                                            <input
                                                type="date"
                                    ...
                                    <div className="space-y-1 pt-1">
                                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-400">Descripción Original:</span>
                                        <p className="text-xs text-slate-600 dark:text-slate-300 max-h-24 overflow-y-auto leading-relaxed bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                                            {selectedCert.descripcion || 'Sin descripción provista por el operador.'}
                                        </p>
                                    </div>
                                </div>
