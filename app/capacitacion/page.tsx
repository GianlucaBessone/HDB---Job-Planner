                                                ...
                                                <button 
                                                    onClick={() => {
                                                        ...
                                                        setEditCertData({
                                                            ...
                                                            horas: cert.horas || '',
                                                            fechaEmision: cert.fechaEmision ? new Date(cert.fechaEmision).toISOString().split('T')[0] : ''
                                                        });
                                                        setIsEditingCert(false);
                                                    }}
                            ...
                            <div className="bg-indigo-50/50 dark:bg-indigo-950/10 p-4 rounded-2xl border border-indigo-100/50 dark:border-indigo-800/40 space-y-4">
                                <div className="flex items-center justify-between">
                                    ...
                                    {previewCert.estado !== 'aprobado' && !isEditingCert && (
                                        <button onClick={() => setIsEditingCert(true)} className="text-[10px] font-bold px-2 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 transition-colors">
                                            Editar Datos
                                        </button>
                                    )}
                                </div>
                                ...
                                {isEditingCert ? (
                                    <div className="space-y-3">
                                        <div className="space-y-1">
                                            ...
                                            <input type="text" value={editCertData.nombreCurso} onChange={(e) => setEditCertData({...editCertData, nombreCurso: e.target.value})} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-200 focus:outline-none focus:border-indigo-500" />
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-slate-500">Institución</label>
                                            <input type="text" value={editCertData.institucion} onChange={(e) => setEditCertData({...editCertData, institucion: e.target.value})} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-200 focus:outline-none focus:border-indigo-500" />
                                        </div>
                                        <div className="grid grid-cols-2 gap-3">
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-bold text-slate-500">Horas</label>
                                                <input type="number" value={editCertData.horas} onChange={(e) => setEditCertData({...editCertData, horas: e.target.value})} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-200 focus:outline-none focus:border-indigo-500" />
                                            </div>
                                            <div className="space-y-1">
                                                <label className="text-[10px] font-bold text-slate-500">Fecha Emisión</label>
                                                <input type="date" value={editCertData.fechaEmision} onChange={(e) => setEditCertData({...editCertData, fechaEmision: e.target.value})} className="w-full px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-200 focus:outline-none focus:border-indigo-500" />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <label className="text-[10px] font-bold text-slate-500">Descripción / Resumen</label>
                                            <textarea value={editCertData.descripcion} onChange={(e) => setEditCertData({...editCertData, descripcion: e.target.value})} className="w-full h-20 px-3 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-200 focus:outline-none focus:border-indigo-500 resize-none" />
                                        </div>
                                        <div className="flex gap-2 justify-end pt-2">
                                            <button onClick={() => setIsEditingCert(false)} className="px-4 py-2 text-xs font-bold text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl transition-colors">Cancelar</button>
                                            <button onClick={handleSaveCertEdit} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl shadow-md transition-colors flex items-center gap-1.5">
                                    ...
                                    <div className="space-y-3 text-xs">
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                ...
                                                <p className="font-bold text-slate-700 dark:text-slate-200">{previewCert.nombreCurso || 'N/A'}</p>
                                            </div>
                                            <div>
                                                <p className="text-slate-400">Institución:</p>
                                                <p className="font-bold text-slate-700 dark:text-slate-200">{previewCert.institucion || 'N/A'}</p>
                                            </div>
                                            <div>
                                                <p className="text-slate-400">Horas:</p>
                                                <p className="font-bold text-slate-700 dark:text-slate-200">{previewCert.horas || 'N/A'}</p>
                                            </div>
                                            <div>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-slate-700 dark:text-slate-200 leading-relaxed mt-0.5">{previewCert.descripcion || 'Sin descripción'}</p>
                                        </div>
                                    </div>
                                )}
